 ...
 * (unfolded) state. The fold factor varies between 0 (completely unfolded) to
 * 1.0 (completely folded, non-visible image).
 *
 * This layout throws an exception if there's more than one child added to the view.
 * For more complicated view hierarchy's inside the folding layout, the views should
 * all be nested inside 1 parent layout.
 *
 * This layout folds the content of its child in real time. By applying matrix
 ...
 * a rectangle. This can be avoided by having the child of this layout wrap
 * its content inside a 1 pixel transparent border. This will cause an anti-aliasing
...
public class FoldingLayout extends ViewGroup {
    ...
    private final String FOLDING_VIEW_EXCEPTION_MESSAGE = "Folding Layout can have " +
            "only 1 child at most";
     ...
     * matrices and values to account for the new fold factor. Once that
     * is completed, it redraws itself with the new fold
     */
     ...
     * and number of folds. This creates the necessary setup in order to prepare
     * the layout for a fold with the specified parameters. Some of the dimensions required
     * for the folding transformation are also acquired here.
     *
    ...
    private void prepareFold(Orientation orientation, float anchorFactor, int numberOfFolds) {
        ...
        int delta = Math.round(mIsHorizontal ? (w / numberOfFolds) : (h / mNumberOfFolds));
         ...
         */
        for (int i = 0; i < mNumberOfFolds; i++) {
            // deltap is the thickness of each fold
            if (mIsHorizontal) {
                int deltap = (i + 1) * delta > w ? w - i * delta : delta;
                mFoldRectArray[i] = new Rect(i * delta, 0, i * delta + deltap, h);
            }
            else {
                int deltap = (i + 1) * delta > h ? h - i * delta : delta;
                mFoldRectArray[i] = new Rect(0, i * delta, w, i * delta + deltap);
            }
        }
    ...
    /**
     * Calculates the transformation matrices used to draw each of the
     * separate folding segments from this view
     */
    private void calculateMatrices() {
        ...
        /** If the fold factor is 1 then the folding view should not be seen
        ...
        // Back to completely unfolded state
        if (mFoldFactor == 0 &&  mPreviousFoldFactor > 0) {
            mFoldListener.onEndFold();
        }

        // Start to fold from a completely unfolded state
